...
import io.onemfive.tor.client.core.circuits.CircuitManagerImpl.CircuitFilter;
...
public class CircuitCreationTask implements Runnable {
	...
	private void expireOldCircuits() {
		final Set<Circuit> circuits = circuitManager.getCircuitsByFilter(new CircuitManagerImpl.CircuitFilter() {
	...
	private int countCircuitsSupportingTarget(final ExitTarget target, final boolean needClean) {
		final CircuitManagerImpl.CircuitFilter filter = new CircuitManagerImpl.CircuitFilter() {
			public boolean filter(Circuit circuit) {
